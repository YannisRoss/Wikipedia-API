class StatisticsBroadcasterJob < ApplicationJob
  ...
  def perform(*args)
    ...
    unless wiki_entries.empty? 
      ...
      statistics[:wordcount_max_min] = wiki_entries.order(:wordcount).limit(1).first.title, wiki_entries.order(wordcount: :desc).limit(1).first.title
      statistics[:biggest_title] = wiki_entries.sort_by{|entry| entry.title}.first.title
      statistics[:lowest_levenshtein] = wiki_entries.sort_by{|entry| entry.levenshtein_distance(entry.title, entry.search_term.body)}.first.title
      statistics[:lowest_levenshtein_term] = wiki_entries.sort_by{|entry| entry.levenshtein_distance(entry.title, entry.search_term.body)}.first.search_term.body
